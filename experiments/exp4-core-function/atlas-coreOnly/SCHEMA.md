...
7. **触发卡禁令有作用域**（v3.1，修法 C）：卡内「禁止」条目只在该卡触发条件命中时生效——compile.py 在 Runtime「情景触发」段头渲染此规则。防 agent 把场景特定禁令（如"绝不全程交易化"限日常脆弱场）搬运成通用禁令（round1-bareR P13 法墙同型错）。
8. **N 编号不进 runtime**（v3.1，round1-bareR 分支1）：N1–N6 是 Atlas-only 引用键（预登记引用 + 链审计映射表，存 core 卡笔记段），不进会编译段——显式列表对判定无增量且诱导撒网挂靠；端的生成元以根（R1/R2）直接渲染。
