# Atlas 卡片字段标准（v3，exp4 双根重组）

> **v3 增量（2026-06-10，本目录 = exp4 新建套，exp3 冻结为历史）**：
> ①core 卡新增**根层 R1/R2**（端的生成元的生成元，正交），旧 N1–N6 降为**定理层**（编号语义不变、每条标根出处 derivation——既有探针预登记引用照旧）；
> ②frontmatter 新增 `verification` 字段（见下表）；
> ③各卡笔记段新增 `derivation` 行（v3 双根下本卡从哪条根/定理推出，标"可全推（压缩候选）/ 含残差（真 π）"）——Atlas-only 元数据，不编译、待人审；
> ④lie 拆「函数形式（π，恒定）/ 防御强度（慢变量参数，随关系阶段）」，作用域限 R1（亲密依恋的暴露）。
...
| `verification` | ✓(v3) | ✗不渲染 | 实证状态：`verified`（行为载荷经 exp1–3 人判闭环实证）/ `declarative`（描述声明，免验证：轴/substrate）/ `pending`（v3 新内容，待纸面体检 → agent 闸；见 ../VERIFICATION.md） |
...
- **根（v3）**：`core` 卡定义里的根编号 **R1/R2**（R1=长寿者的孤独诅咒〔双极〕、R2=长寿智者的正面骄傲）。根之间正交（互不推导）；同一情景要素可同时激活双根。
- **端的定理层**：`core` 卡定义里的常驻需求编号 **N1…Nk**（如 N1=被当作她自己看见）。v3 起每条 N 是根的定理（标 ⟸ 出处）；**挂 N 等价于挂其根，直接挂 R 同样合法**（N 是高频缓存，不是值域全集）。
- **法的生成元**：`output-bans` 的姿态约束编号 **P1…Pk**。
- 编号**只是稳定引用键**（链引"哪条需求/哪条约束"用），不改语义、不新增机制；改号=破坏既有链引用，禁随意重编。v3 降格 N 层时编号未动正因此。
