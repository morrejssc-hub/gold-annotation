...
async function runVolume(vol) {
  ...
  const summary = await agent(
    ...
    `=== claude 臂 核查结论 ===\n${chkC}\n\n=== claude 臂 对抗结论 ===\n${advC}\n\n=== glm 臂 核查结论 ===\n${chkG}\n\n=== glm 臂 对抗结论 ===\n${advG}\n\n` +
    `产出两个文件：\n` +
    `(A) anchors/notebook/对比总结_卷${vol}_glm_vs_claude.md，含：①两臂范围切法差异 ②读法收敛/分歧/互补（哪些双臂都说且都扛过对抗=高置信，哪些分歧需人裁，哪些互补可拼） ③并置才浮出的新读法（重点） ④每条读法标置信度 高/中/存疑（依据=是否收敛+是否扛过对抗，事实瑕疵/虚构引核查结论） ⑤笔记思路（要写 notebook 架构入口的话主轴怎么串、拆哪几个主题原子）。开头标"非定稿,裁决/升格/定稿归人"。\n` +
    `(B) anchors/notebook/对比总结_卷${vol}_glm_vs_claude_核查对抗附录.md：把上面收到的四份核查/对抗结论**原样搬运**存档（四个二级标题 ## claude臂核查 / ## claude臂对抗 / ## glm臂核查 / ## glm臂对抗），开头一行注"原始报告存档,非定稿"。不改写内容。\n` +
