...
2. **本段之前已经在场的东西**：前史与关系档位，缺了它就看不懂本卷动作。把前几卷的旧伤/线索带进来（如"孤独会要人命""别替我安排退场""直接把真心话说出来""把我看得够重"；卷02 定稿核："公共善下的特别性追问"——他对很多人好、赫萝要确认自己不是顺手被善待的一个；"不同善意=被依靠/托付/接受其可怕一面，而非被保护"；"特别性预设同行延续，故先保延续再压特别性"）。
3. **这几幕直接发生了什么**：事实层。**每个关键动作都回指 scene + 行号**，一般引用保持短摘；**但承重句（你某条读法的关键证据）要引足到结构完整、能自证**——别只摘命中读法的那半句，相邻分句常扛着真正的分量或反证（卷02 s46 只摘"不再恐惧"、漏掉同句的"赫萝只要轻轻用力就能推倒他"，差点弱化了强弱线最硬的证据）。
4. **值得盯住的细节**：本卷特有的别扭、不对称、与前几卷同核异形的地方。
5. **暂时并存的读法**：允许多解并存，只呈现解释路径，不裁决哪个是本质。**但若你反而收敛出一条强主轴/统一读法，必须同时给它划边界**：(a) 对"同一套说法在不同 scene 的功能"做分化——同一词汇/手段在各 scene 干的事往往不同，别压扁成一句万能解释（卷01 的压舱石就是一张"债/契约按 scene 功能分化表"）；(b) 标明哪些主题只是被它"暂时兜住"、各自另有独立动力，主轴是中介语法、不是所有主题的生成根。强读法越漂亮越要防它吞并散点。(c) **不同 scene 的伤/机制可能落在不同轴上，别因为它们服务同一主轴就焊成一条**——连接处要有文本证据，不能靠你叠加的解释性 gloss（卷02 把留盘缠焊到强弱线就是弱焊：留盘缠的伤在"独自上路/共同面对"轴、强弱线在"被依靠"轴，两者只在主轴伞下并列）。
...
- **分清"现场文本确立的"与"后验/跨卷知识"**：判断角色当场的把握/意图只用本场文本，别拿后面卷的结果倒灌回当场当成既定事实（卷01："赫萝押上生死赌他来救"多是罗伦斯的现场读法，她当场表达的是把握、文本没说死）。
...
- **核对每句话是谁说的**：别把 B 角色的话算到 A 头上——卷01 第一稿把罗伦斯那句"永远还不清的人情"误当成赫萝造的胁迫筹码，把她的手法读成威胁就失真了。写完连同 scene 号一起亲核"这话是谁说的"。
- **校准强度，别拔高（第一稿系统性高估是反复的坑）**：读法用词分清"结构对称 vs 文本分量对称""先给信任押注/识人判断 vs 已是高信任""关系行为的重要底色 vs 全部行为的底色"；别让一句话的鲜活标签盖过架构口径（如零点 trust 档仍是负数/试探，哪怕她当场显得自信）；别用"赴死"这类戏剧化措辞替代"冒险/有把握的行动"。卷01 评审在这类拔高上逐处拉回。
- **别预设本卷一定有 trust 档跃迁或恋爱里程碑式升温。** 关系进展可能落在非显性轴上（如关系内可追讨权上升、关系 mode 从"被保护"迁到"被依靠"）。按"这卷的升温该长什么样"去硬找，会把真实进展读漏、或把过场句（卷02 s46 那句"不再恐惧"）误抬成里程碑。先认本卷自己的进展形态。
